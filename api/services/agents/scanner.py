...
class ScannerAgent(AbstractAgent):
    async def run(self, input: ScannerInput) -> ScannerOutput:
        ...
        for position, inci_name in enumerate(input.product.ingredients_parsed):
            # 1. Check our ingredients DB (has hazard_tags from seed data)
            rows = (
            ...
            # 2. Check EU banned list — only call LLM if ingredient is banned
            if is_banned(inci_name):
                result = await llm_client.classify_ingredient(inci_name, SCANNER_PROMPT)
                flagged.append(
                    FlaggedItem(
                        inci_name=inci_name,
                        position=position,
                        hazard_tags=result.get("hazard_tags", ["eu_banned"]),
                        known_in_db=False,
                    )
                )
